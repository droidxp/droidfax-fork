...
do
	...
	#rt=`cat lowcov_malware | awk '{print $1}' | grep -a -c "^${i}.apk.logcat$"`
		...
		sh securityReport_rakib.sh \
			$orgapk \
			$TRACEDIR/$packname.logcat $RESULTDIR/securityReport >> $resultlog 2>&1
	#fi
done

#mv /home/hcai/testbed/{srcsink.txt,src.txt,sink.txt,callback.txt,lifecycleMethod.txt,eventHandler.txt} $RESULTDIR/securityReport/
#mv /home/hcai/testbed/securityfeatures.txt $RESULTDIR/securityReport
