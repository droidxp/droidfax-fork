...
public class GatorHierarchy {
  ...
  public boolean isCallback (SootMethod m, SootClass c) {
	  if (frameworkManaged.get(c)==null) return false;
	  if (! frameworkManaged.get(c).contains(m)) return false;
	  
	  SootMethod cb = virtualDispatch (m.getSubSignature(), c);
  ...
  // ------------------
  private static GatorHierarchy instance;

  public static synchronized GatorHierarchy v() {
    if (instance == null) {
      instance = new GatorHierarchy();
    }
    return instance;
  }

  private GatorHierarchy() {
    Scene scene = Scene.v();
...
//    }
  ...
  void buildApplicationCodeMap() {
    Logger.verb("TIMECOST", "buildApplicationCodeMap parsing starting at " + System
            .currentTimeMillis());
    ...
    Logger.verb("TIMECOST", "buildApplicationCodeMap parsing finished at " + System
            .currentTimeMillis());
    
    Logger.verb("Instantiated app classes", "total number: " + instantiatedClasses.size());
  }
